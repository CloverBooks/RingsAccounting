...
- [ ] Option 2 left on the table after this restored-runtime slice: replace the remaining static top-bar environment chrome and dead quick actions in `AdminApp` such as the fixed prod badge and `View error traces` button with runtime-settings-backed values or remove them.
