...
- [ ] Option 2 left on the table after this backend hardening slice: standardize deterministic admin mutation error envelopes and error codes across the remaining privileged `/api/admin/*` routes once the current phase is closed.
...
- [ ] Option 2 left on the table after this audit-log contract slice: remove the remaining garbled icon/text mojibake in the canonical admin UI and replace it with ASCII-safe copy or proper icon components so operators do not see corrupted chrome.
