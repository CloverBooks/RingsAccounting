...
import { fetchAutonomyStatus, fetchUsers } from "./api";
...
describe("admin api trace ids", () => {
  ...
  it("appends response header request ids to autonomy API failures", async () => {
    globalThis.fetch = vi.fn().mockResolvedValue(
      new Response(JSON.stringify({ detail: "engine unavailable" }), {
        status: 503,
        headers: {
          "content-type": "application/json",
          "x-request-id": "companion-trace-01",
        },
      }),
    ) as typeof fetch;

    await expect(fetchAutonomyStatus(42)).rejects.toMatchObject({
      status: 503,
      requestId: "companion-trace-01",
      message: "engine unavailable (Request ID: companion-trace-01)",
    });
  });
});
